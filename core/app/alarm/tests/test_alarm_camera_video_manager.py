...
class AlarmCameraVideoManager(TestCase):
    def setUp(self) -> None:
        ...
        self.videos = (
            CameraMotionVideoFactory(device=self.device, event_ref=self.motions[0].event_ref),
            CameraMotionVideoFactory(device=self.device, event_ref=self.motions[1].event_ref, number_records=1),
        )
        return super().setUp()
