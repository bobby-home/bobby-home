...
from alarm.use_cases.camera_motion import CameraMotion
...
class CameraMotionTestCase(TestCase):
    def setUp(self) -> None:
        ...
        self.record_manager_mock = Mock(spec_set=AlarmCameraVideoManager)
        self.camera_motion = CameraMotion(self.record_manager_mock)
    ...
    def test_camera_motion_detected(self, integration_camera_motion, save_motion_mock):
    ...
    def test_camera_motion_no_more_motion(self, integration_camera_no_more_motion, integration_alarm_status_changed, save_motion_mock):
        in_data = self._get_in_motion_camera_data(status=True)
        self.camera_motion.motion_detected(in_data)
        self._assert_save_motion(save_motion_mock, status=True)
        ...
        self.camera_motion.motion_detect_ended(in_data)
