...
class OnUpdateStatusTestCase(TestCase):
    ...
    @patch('alarm.mqtt.on_status_update.AlarmChangeStatus', spec_set=AlarmChangeStatus)
    ...
    def test_specific_device(self, alarm_change_status_mock):
        ...
        alarm_change_status_mock.all_change_statuses.assert_not_called()

    @patch('alarm.mqtt.on_status_update.AlarmChangeStatus', spec_set=AlarmChangeStatus)
    def test_all_device(self, alarm_change_status_mock):
        data_payload = UpdateStatusPayload(status='on', force='on')
        self.update.on_update_all(data_payload)
        alarm_change_status_mock.all_change_statuses.assert_called_once_with(status=True, force=True)
