...
def camera_object_detection_factory(device_id: str, camera_analyze_object: CameraAnalyzer, camera_recorder: CameraRecorder, detect_people: Optional[DetectPeople] = None) -> CameraObjectDetection:
    if detect_people is None:
        detect_people = detect_people_factory()

    return CameraObjectDetection(camera_analyze_object, detect_people, get_mqtt, device_id, CameraRecording(device_id, camera_recorder))
