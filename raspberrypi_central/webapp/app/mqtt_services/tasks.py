...
def verify_service_status(device_id: str, service_name: str, status: bool, since_time) -> None:
    if not is_in_status_since(device_id, service_name, status, since_time):
        if status:
            send_message(_(f'Your service {service_name} should turn on but is still off. Something is wrong.'))
        else:
            send_message(_(f'Your service {service_name} should turn off but is still up. Something is wrong.'))


@shared_task()
def mqtt_status_does_not_match_database(model_ref: models.Model, device_id: str, received_status: bool, service_name: str):
    if received_status:
